...
-- ============================================================
-- PAGE LAYOUTS (admin can customize homepage sections, ordering, visibility)
-- ============================================================
create table if not exists public.page_sections (
  id bigint generated by default as identity primary key,
  page text not null default 'home' check (page in ('home','videos','photos')),
  section_type text not null check (section_type in ('hero','featured','trending','recent','categories','tags','featured-photos','custom')),
  title text,
  description text,
...
create policy "page_sections_select_public" on public.page_sections for select using (visible=true or public.is_vexa_admin());
drop policy if exists "page_sections_admin_write" on public.page_sections;
create policy "page_sections_admin_write" on public.page_sections for all to authenticated
using (public.is_vexa_admin()) with check (public.is_vexa_admin());

insert into public.page_sections (page,section_type,title,display_order,visible) values
('home','hero','Welcome',0,true),
('home','featured','Featured Videos',1,true),
('home','trending','Trending Now',2,true),
('home','recent','Recent Uploads',3,true),
('videos','featured','Featured Videos',0,true),
('videos','categories','Browse Categories',1,true),
('photos','featured-photos','Featured Photos',0,true)
