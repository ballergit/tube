...
-- VEXA MEDIA UPLOAD / PHOTO STORAGE SETUP
-- Run this in Supabase SQL Editor after the existing backend.sql/admin.sql.
...
for update to authenticated using (bucket_id = 'media' and owner_id = auth.uid()::text) with check (bucket_id = 'media');

drop policy if exists "users can delete own media" on storage.objects;
create policy "users can delete own media" on storage.objects
for delete to authenticated using (bucket_id = 'media' and owner_id = auth.uid()::text);
...
create policy "published photos are public" on public.photos for select using (status='published' or auth.uid()=user_id);
...
create policy "users can update own photos" on public.photos for update to authenticated using (auth.uid()=user_id) with check (auth.uid()=user_id);
