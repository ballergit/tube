...
-- ============================================================
-- PROFILE CORRECTION: bio, social link, profile media, counts
-- ============================================================
alter table public.profiles add column if not exists bio text;
alter table public.profiles add column if not exists social_link text;

-- Public profile data contains no email/admin-sensitive fields.
drop view if exists public.vexa_public_profiles;
create or replace view public.vexa_public_profiles as
...
-- Public profile media bucket. Users can write only inside their own folder.
...
create policy "vexa_profile_assets_public_read" on storage.objects
for select using (bucket_id='profile-assets');

drop policy if exists "vexa_profile_assets_insert_own" on storage.objects;
create policy "vexa_profile_assets_insert_own" on storage.objects
...
create policy "vexa_profile_assets_update_own" on storage.objects
for update to authenticated
...
create policy "vexa_profile_assets_delete_own" on storage.objects
for delete to authenticated
using (bucket_id='profile-assets' and (storage.foldername(name))[1]=auth.uid()::text);

-- ============================================================
-- END PROFILE CORRECTION
-- ============================================================
